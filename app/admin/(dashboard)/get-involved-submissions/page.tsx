...
import { Search, RefreshCw, Eye, HeartHandshake } from "lucide-react";
...
export default function GetInvolvedSubmissionsPage() {
  const [submissions, setSubmissions] = useState<GetInvolvedSubmission[]>([]);
  ...
  const [selected, setSelected] = useState<GetInvolvedSubmission | null>(null);
  ...
  const fetchData = async (opts?: { pageOverride?: number }) => {
    ...
    try {
      ...
      if (topicFilter !== "All") params.set("topic_area", topicFilter);
      const res = await adminFetch(`/api/admin/get-involved-submissions?${params.toString()}`);
      ...
      try {
        const [totalRes, newRes] = await Promise.all([
          adminFetch(`/api/admin/get-involved-submissions?limit=1`).then((r) => (r.ok ? r.json() : null)),
          adminFetch(`/api/admin/get-involved-submissions?status=new&limit=1`).then((r) => (r.ok ? r.json() : null)),
        ]);
        setStats({
  ...
  const filtered = submissions.filter((s) => {
    ...
    return (
      ...
      s.topic_area.toLowerCase().includes(q) ||
      s.message.toLowerCase().includes(q)
    );
  });

  const openDetail = (item: GetInvolvedSubmission) => {
    setSelected(item);
    setDetailError(null);
  ...
  const updateStatus = async (newStatus: string) => {
    ...
    setDetailSuccess(null);
    try {
      const res = await adminFetch(`/api/admin/get-involved-submissions/${selected.id}`, {
        method: "PATCH",
      ...
      const updated = data.submission as GetInvolvedSubmission;
      setSelected(updated);
    ...
    <div>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">Manage Share Your Voice submissions</p>
          {stats.total !== null && (
      ...
      <div className="flex flex-col lg:flex-row gap-3 mb-4">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <select
            ...
            className="px-3 py-2.5 bg-slate-800/50 border border-slate-700/50 rounded-lg text-white text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/50 max-w-[200px]"
          >
            {TOPIC_AREAS.map((t) => (
      ...
      {loading ? (
        ...
        <div className="bg-red-500/10 border border-red-500/20 text-red-400 px-4 py-4 rounded-xl text-sm">{error}</div>
      ) : filtered.length === 0 ? (
        <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-12 text-center">
          <HeartHandshake className="w-10 h-10 text-slate-500 mx-auto mb-3" />
          <p className="text-slate-400 text-sm">
            {search || topicFilter !== "All" || statusFilter !== "All"
              ? "No matching Get Involved submissions found."
              : "No Get Involved submissions yet."}
          </p>
        </div>
      ) : (
        <>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-slate-700/50">
                    ...
                    <th className="text-left text-xs font-semibold text-slate-400 uppercase tracking-wider px-6 py-4">Topic Area</th>
                ...
                <tbody className="divide-y divide-slate-700/30">
                  {filtered.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-700/20 transition-colors">
                      ...
                      <td className="px-6 py-4 text-sm text-slate-300 whitespace-nowrap">{item.topic_area}</td>
            ...
            {filtered.map((item) => (
              <div key={item.id} className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-4">
                ...
                <div className="text-xs text-slate-400 mb-1">
                  {item.topic_area} • {new Date(item.created_at).toLocaleDateString()}
                </div>
          ...
          <div className="bg-slate-800 border border-slate-700/50 rounded-2xl w-full max-w-2xl shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex items-start justify-between gap-4 mb-4">
                <h3 className="text-lg font-semibold text-white">Get Involved Submission</h3>
                <button onClick={() => setSelected(null)} className="text-slate-400 hover:text-white text-xl leading-none">
                  ×
                </button>
              </div>
              ...
              <div className="space-y-3 text-sm">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    <div className="text-xs text-slate-400 uppercase tracking-wider">Topic Area</div>
                    <div className="text-white">{selected.topic_area}</div>
                  </div>
                  <div>
                ...
                <div>
                  <div className="text-xs text-slate-400 uppercase tracking-wider mb-1">Insights / Proposal</div>
