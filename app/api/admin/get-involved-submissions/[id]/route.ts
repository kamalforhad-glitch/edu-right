...
import {
  getGetInvolvedSubmissionById,
  updateGetInvolvedSubmissionStatus,
} from "@/lib/models/GetInvolvedSubmission";
...
export async function GET(
  ...
  try {
    const submission = await getGetInvolvedSubmissionById(id);
    if (!submission) return privateJson({ error: "Not found" }, { status: 404 });
    return privateJson({ submission });
  } catch (error) {
    console.error("Get get-involved submission error:", error);
...
export async function PATCH(
  ...
  const rl = await checkRateLimit(request, {
    namespace: "admin_get_involved_patch:user",
  ...
  try {
    ...
    const updated = await updateGetInvolvedSubmissionStatus(id, status);
    ...
    console.error("Update get-involved submission error:", error);
